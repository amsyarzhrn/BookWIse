...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Retrieve all resource IDs along with their details
    $sql = "SELECT c.resourceID, c.resourceName, c.resourceType FROM resourceinfo c";
    ...
    foreach ($resources as $resource) {
        ...
        $stmt->bind_param("sssssss", $resourceID, $bookingStartTime, $bookingStartTime, $bookingStartTime, $bookingEndTime, $bookingEndTime, $bookingEndTime);
        $stmt->execute();
        $result = $stmt->get_result();
        $existingBookings = $result->fetch_all(MYSQLI_ASSOC);
        
        // If no overlapping bookings, the resource is available
    ...
    if (!empty($availableResources)) {
        $availabilityFeedback = "The following resource are available for the selected date and time range:";
    } else {
        $availabilityFeedback = "<span class='error'>There are no resources available for the selected date and time range.</span>";
    }
}
?>
...
/* Hover effect for the Book Resource button */
...
<div class="buttonContainer">
        <a href="bookresource.php" class="myButton">Book Resource</a>
    </div>

 <main>

        <br>
        <h1>Search Available Resource</h1><br><br>
    ...
    <?php echo $availabilityFeedback; ?></p>
            ...
            <?php foreach ($availableResources as $resource) : ?>
                <tr>
                    <td><?php echo $resource['resourceID']; ?></td>
                     <td><?php echo $resource['resourceType']; ?></td>
        ...
        <form action="bookresource.php" method="get">
