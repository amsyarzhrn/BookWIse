...
$stmt->bind_param("ssssssss", $bookingDate, $resourceID, $bookingStartTime, $bookingStartTime, $bookingStartTime, $bookingEndTime, $bookingEndTime, $bookingEndTime);
$stmt->execute();
    ...
    if (count($existingBookings) > 0) {
        ...
        $availabilityFeedback = "<span class='error'>The selected resource is not available for the selected date and time range. Please try again </span>";
    } else {
        ...
        $insertSql = "INSERT INTO booking (bookingID, resourceID, userUsername, bookingDate, bookingStartTime, bookingEndTime, bookingDuration)
                      VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insertSql);
        $stmt->bind_param("sssssss", $bookingID, $resourceID, $_SESSION['username'], $bookingDate, $bookingStartTime, $bookingEndTime, $_POST['bookingDuration']);
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $availabilityFeedback = "<span class='success'>Booking successful. Your booking ID is: " . $bookingID . ".<br> Show your booking ID at the admin counter upon resource collection.<br> Please refer to your booking details at <a href='mybookings.php'>My Bookings</a>.</span>";
        } else {
            // Booking failed
        ...
        <nav>
            ...
            <div class="nav-links" id="navLinks">
                <ul>
                    ...
                    <li class="active"><a href="bookresource.php"><span class="material-symbols-outlined">
                    edit_calendar
                    </span> Booking Resource</a></li>
...
<!-- End webpage header html-->
    ...
    <div class="buttonCourt">
        <a href="resourceavailability.php" class="ViewCourt">Search Resources</a>
    </div>
 ...
 <main>
        ...
        <table>
            ...
            <tr>
             ...
             <tr>
                <td >
                    <label for="resourceID">Select Resource: </label><br>
                    <span>(Resource Name - Resource Type )</span>
                </td>
                <td>
                    <?php
                    // SQL query to fetch court information
                    $courtInfoQuery = "SELECT * FROM resourceinfo";
                    $courtInfoResult = mysqli_query($conn, $courtInfoQuery);
                    echo "<select id='resourceID' name='resourceID'>";
                    while ($row = mysqli_fetch_assoc($courtInfoResult)) {
                        echo "<option value='" . $row['resourceID'] . "'> " . $row['resourceName'] . "  - " . $row['resourceType'] . "</option>";
                    }
                    echo "</select>";
                    ?>
                </td>
            ...
            <tr>
                <td colspan=2><input type="submit" value="BookResource">  
            </tr>
        </table>
        <br><br>
